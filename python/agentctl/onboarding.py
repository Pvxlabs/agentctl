...
DEFAULT_ADAPTER_FILE = "agentctl_trusted_access_adapter.py"

_ADAPTER_SCAFFOLD = '''"""DEV-only identity bootstrap seam generated by agentctl.

Replace this module with the application's own adapter when the project has a
real DEV identity store. It intentionally stores no passwords and is limited
to the local DEV identity state used by onboarding.
"""
...
class Adapter:
    def __init__(self) -> None:
        self._path = Path(__file__).resolve().parent / ".agentctl" / "trusted-access-identities.json"
    ...
    def ensure_identity(self, account: str, role: str, **_kwargs: Any) -> None:
        identities = self._read()
        current = identities.get(account, {})
        identities[account] = {"active": current.get("active", True), "role": current.get("role", role)}
        self._write(identities)

    def validate_role(self, account: str, role: str) -> bool:
        state = self.inspect_identity(account)
        return isinstance(state, Mapping) and state.get("role") == role

    def validate_active(self, account: str) -> bool:
        state = self.inspect_identity(account)
        return isinstance(state, Mapping) and state.get("active") is True
...
def _write_adapter_scaffold(path: Path) -> None:
    if path.exists():
        return
    temporary = path.with_suffix(path.suffix + ".tmp")
    temporary.write_text(_ADAPTER_SCAFFOLD, encoding="utf-8")
    temporary.replace(path)
...
def _identity_checks(facts: ProjectFacts, manifest: ProjectManifest, *, execute: bool) -> dict[str, Any]:
    ...
    for name, item in profile.items():
        ...
        result[name] = {
            ...
            "status": "PASS" if state is not None and active and role else "FAIL",
            "active": active,
            "role": item.role,
            "created": created,
        }
...
def build_onboarding_plan(path: str | Path = ".") -> OnboardingResult:
    ...
    return OnboardingResult(
        ...
        integration={"status": "scaffold_required", "adapter": DEFAULT_ADAPTER_MODULE},
        smoke={"status": "after_integration"},
        changes=["create .agent-control.yaml", f"create {DEFAULT_ADAPTER_FILE}", "bootstrap canonical DEV runtime", "ensure DEV identities"],
    )
...
def onboard(path: str | Path = ".", *, runtime_dir: str | Path | None = None, plan: bool = False) -> dict[str, Any]:
    ...
    if facts.manifest is None:
        ...
        if not plan:
            ...
            adapter_path = facts.root / DEFAULT_ADAPTER_FILE
            if not adapter_path.exists():
                _write_adapter_scaffold(adapter_path)
    ...
    return OnboardingResult(
        ...
        changes=(["create .agent-control.yaml"] if created_manifest else [])
        + ([f"create {DEFAULT_ADAPTER_FILE}"] if created_adapter else [])
        + (["update DEV profile metadata"] if changed else [])
        + (["bootstrap canonical DEV runtime"] if not runtime_status.get("runtime_ready") else [])
        + (["ensure DEV identities"] if any(item.get("created") is True for item in identities.values()) else []),
    ).to_dict()
